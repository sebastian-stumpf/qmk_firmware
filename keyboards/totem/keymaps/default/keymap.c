#include QMK_KEYBOARD_H
#include "totem.h"
#include "keymap_german.h"

enum layers { BASE, SYM, NUM, FUN, NAV };

#define A_LT LGUI_T(DE_A)
#define S_LT LALT_T(DE_S)
#define D_LT LCTL_T(DE_D)
#define F_LT LSFT_T(DE_F)
#define J_LT LSFT_T(DE_J)
#define K_LT LCTL_T(DE_K)
#define L_LT LALT_T(DE_L)
#define HASH_LT LGUI_T(DE_HASH)


#define DEL_NUM LT(NUM, KC_DEL)
#define BSPC_FUN LT(FUN, KC_BSPC)
#define SPC_NAV LT(NAV, KC_SPC)
#define TAB_SYM LT(SYM, KC_TAB)


#define OS_GUI OSM(MOD_LGUI)
#define OS_ALT OSM(MOD_LALT)
#define OS_CTL OSM(MOD_LCTL)
#define OS_SFT OSM(MOD_LSFT)

#define INS_MODE LSA(KC_INS)

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
   [BASE] = LAYOUT(
                 DE_Q,    DE_W,    DE_E,    DE_R,    DE_T,         DE_Z,    DE_U,     DE_I,    DE_O,    DE_P,
                 A_LT,    S_LT,    D_LT,    F_LT,    DE_G,         DE_H,    J_LT,     K_LT,    L_LT,    HASH_LT,
        KC_CALC, DE_Y,    DE_X,    DE_C,    DE_V,    DE_B,         DE_N,    DE_M,     DE_COMM, DE_DOT,  DE_MINS, TG(NUM),
                          KC_ESC,  DEL_NUM, BSPC_FUN,     SPC_NAV, TAB_SYM,  KC_ENT
    ),
    [SYM] =  LAYOUT(
                 DE_AT,   DE_LPRN, DE_RPRN, DE_UDIA, DE_SS,        XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
                 DE_BSLS, DE_LCBR, DE_RCBR, DE_ADIA, DE_ODIA,      XXXXXXX, KC_LSFT, KC_LCTL, KC_LALT, KC_LGUI,
        _______, DE_EURO, DE_LBRC, DE_RBRC, DE_CIRC, DE_GRV,       XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, _______,
                          _______, _______, _______,      _______, _______, _______
    ),
    [NUM] =  LAYOUT(
                 XXXXXXX, XXXXXXX,    C(KC_PGUP), C(KC_PGDN), XXXXXXX,      DE_7,    DE_8,    DE_9,    DE_TILD, TO(BASE),
                 KC_LGUI, KC_LALT,    KC_LCTL,    KC_LSFT,    XXXXXXX,      DE_4,    DE_5,    DE_6,    DE_PIPE, DE_LABK,
        _______, XXXXXXX, C(S(DE_X)), C(S(DE_C)), C(S(DE_V)), XXXXXXX,      DE_1,    DE_2,    DE_3,    DE_QUES, DE_PLUS, TO(BASE),
                             _______,    _______, _______,         _______,  DE_0, _______
    ),
    [FUN] =  LAYOUT(
                 OS_GUI,  OS_ALT,  OS_CTL,  OS_SFT,  XXXXXXX,      KC_F7,    KC_F8,   KC_F9,   KC_F12,  TG(NUM),
                 KC_LGUI, KC_LALT, KC_LCTL, KC_LSFT, XXXXXXX,      KC_F4,    KC_F5,   KC_F6,   KC_F11,  XXXXXXX,
        _______, XXXXXXX, C(DE_X), C(DE_C), C(DE_V), XXXXXXX,      KC_F1,    KC_F2,   KC_F3,   KC_F10,  XXXXXXX, _______,
                          _______, _______, _______,      _______, _______, _______
    ),
    [NAV] =  LAYOUT(
                 CW_TOGG, KC_PGUP, KC_HOME, KC_UP,   KC_END,       KC_VOLU, KC_MPRV, KC_MPLY, KC_MNXT, XXXXXXX,
                 KC_CAPS, KC_PGDN, KC_LEFT, KC_DOWN, KC_RIGHT,     KC_MUTE, KC_LSFT, KC_LCTL, KC_LALT, KC_LGUI,
        _______, KC_INS,  INS_MODE,XXXXXXX, XXXXXXX, XXXXXXX,      KC_VOLD, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, _______,
                          _______, _______, _______,      _______, _______, _______
    )
};
